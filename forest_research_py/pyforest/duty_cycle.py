...
CONFIG = {
    'random_state': 42,
    'csv_name': "D:\\2014\\Forest\\forest_research\\Kvinta.Forest.Compute.HOG\\HOG48-9.csv",
    ...
    'desk_names' : 'HOG64-9-',
    'duty_cycle': ['100','56','35','25','18','14','11','8','6']
...
for dsk_name in CONFIG['duty_cycle']:
    roc_data[dsk_name] = dict()
    pr_data[dsk_name] = dict()
    det_data[dsk_name] = dict()

for dsk_name in CONFIG['duty_cycle']:
    csv_data = os.path.join(CONFIG['path'],CONFIG['desk_names'] + dsk_name + '.csv')
    # читаем из файла
    ...
    model_rfc = RandomForestClassifier(n_estimators = 128)
    for score in CONFIG['scorings']: # цикл по метрикам
        values = cross_validation.cross_val_score(model_rfc, X, y, cv=kf, scoring=score, n_jobs=1)
        mean = values.mean()
        std = values.std()
...
# рисуем roc кривые
plt.subplot(2, 2, 1)
for n in CONFIG['duty_cycle']: # цикл по дескрипторам
    data = roc_data[n]
...
plt.subplot(2, 2, 4)
group_index = np.arange(len(CONFIG['duty_cycle']))
group_names = [i for i in CONFIG['duty_cycle']]
...
plt.xlabel(u'Коэффициент заполнения, %')
plt.ylabel(' ')
